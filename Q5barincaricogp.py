...
df_men = pd.read_csv("CgpincaricoMA.csv")
df_women = pd.read_csv("CgpincaricoFE.csv")
...
def plot_bar_chart(legislatura):
    ...
    men_counts = men_filtered.groupby('gruppoParlamentare')['count'].sum()
    women_counts = women_filtered.groupby('gruppoParlamentare')['count'].sum()
