...
data = pd.read_csv('partyspettro.csv')
...
for y, party in zip(y_pos, party_gender_counts.index):
    ax.text(0.5, y, party, ha='center', va='center', fontsize=7)

    # Aggiungi l'annotazione del testo per l'allineamento politico
    alignment = data[data['partito'] == party]['Allineamento Politico'].values[0]
    ax.annotate(alignment, xy=(0.5, y), xytext=(10, 0), textcoords="offset points", ha='left', va='center', fontsize=7, color='gray')
